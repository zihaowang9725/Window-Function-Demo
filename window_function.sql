...
# The OVER clause defines window partitions to form the groups of rows specifies the orders of rows in a partition. 
...
select 
    ...
    ) as lowest_salary_of_all
from employees e;

#########################################First_value##########################################################################
# The following statement returns the employees (first name) who have the lowest salary in each department.
...
with tablea as (select
    ...
    order by salary desc) as lowest_salary_of_the_dep
from employees)
select *
from tablea
where lowest_salary_of_the_dep = 1
;
...
#########################################CUME_DIST()##########################################################################
# The following statement finds the cumulative distribution values 
# by headcount of each department_id
select *
from employees;

## using temporary table
with tablea as (
select department_id, count(*) head_count
from employees
group by department_id
)
select department_id, head_count,
	round(cume_dist() over(
    order by head_count), 2) as cumunative_dis
from tablea;
...
select first_name, last_name, salary,
	dense_rank() over (
    order by salary desc) salary_ranking
from employees;
...
#########################################RANK()##########################################################################
#below query rank the user based on the salary
SELECT 
	first_name, 
	last_name, 
	salary, 
...
#below query rank the user based on the salary and department_id
...
#compare the rank() and dense_rank()
select first_name, last_name, department_id, 
	rank() over(
    
	order by department_id) as ranking,
    dense_rank() over(
    
    order by department_id) as dens_ranking
from employees;
...
#pagination--select the Nth to Nth rows
WITH t AS(
    SELECT 
        ROW_NUMBER() OVER (
            ORDER BY salary
...
with tablea as (select row_number() over(
    partition by department_id
